...
const Textarea = React.forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ className, ...props }, ref) => (
    <textarea
      className={cn(
        "pib-textarea flex w-full rounded-md border-none bg-transparent px-3 py-2.5 text-base text-gray-100 placeholder:text-gray-400 focus-visible:outline-none focus-visible:ring-0 disabled:cursor-not-allowed disabled:opacity-50 min-h-[44px] resize-none",
        className,
      )}
      ref={ref}
      rows={1}
...
/* ─── Dialog primitives (image preview) ──────────────────────────── */
...
/* ─── Inline Button (kept local — not the shadcn one) ────────────── */
...
const VoiceRecorder: React.FC<VoiceRecorderProps> = ({
  ...
  // Stable per-mount randomized bar shapes (heights + animation timings).
  // Computed once so React doesn't reshuffle them every render.
...
/* ─── Subtle divider between mode toggles ────────────────────────── */
const CustomDivider: React.FC = () => (
  <div className="pib-divider" aria-hidden="true" />
);
...
interface PromptInputBoxProps {
  /** Send button handler. Receives the typed message and any attached files. */
  onSend?: (message: string, files?: File[]) => void;
  /** Optional callback fired the moment a file is added (so the parent can
   *  trigger upload / cost calculation / scan preparation immediately). */
...
export const PromptInputBox = React.forwardRef(
  (props: PromptInputBoxProps, ref: React.Ref<HTMLDivElement>) => {
    ...
    const handleStartRecording = () => {
      // No-op for now — recording is local-only until a backend is wired
    };
      ...
      <>
        ...
        >
          ...
          <PromptInputActions className="flex items-center justify-between gap-2 p-0 pt-2">
            ...
            >
              {/* Hidden file input — kept as a sibling so it isn't nested
                  inside an interactive button (a11y rule) */}
              <input
                ref={uploadInputRef}
                type="file"
                className="hidden"
