          ...
          {stats.map((stat, index) => (
            ...
            >
              <div className="hfs-icon-tile relative w-11 h-11 rounded-2xl flex items-center justify-center transition-all duration-300 group-hover:scale-110 group-hover:-translate-y-0.5">
                ...
                />
                <stat.icon
                  className="relative w-[18px] h-[18px] text-white/60 transition-colors duration-300 group-hover:text-[#d96c4a]"
                  strokeWidth={1.75}
                />
              </div>
              <span className="text-[14px] font-bold text-white/80 whitespace-nowrap tracking-tight">
                {stat.text}
              </span>
            </motion.div>
          ))}
