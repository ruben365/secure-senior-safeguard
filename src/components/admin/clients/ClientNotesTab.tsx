...
import { Plus, Search, Edit, Trash2, Pin, AlertTriangle, FileText } from "lucide-react";
import { useClientNotes, useDeleteClientNote, type ClientNote } from "@/hooks/useClientNotes";
...
export function ClientNotesTab({ clientId }: ClientNotesTabProps) {
  ...
  const filteredNotes = notes
    ...
    .filter(note => {
      ...
      if (filterTag === "important") return note.importance === "urgent" || note.importance === "high";
      return note.tags?.some(t => t.toLowerCase().replace(/ /g, "-") === filterTag);
    })
    .sort((a, b) => {
      if (sortBy === "newest") {
        ...
        <CardContent className="space-y-2">
          {filteredNotes.length === 0 ? (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p>No notes found</p>
              <p className="text-sm">Add a note to track important information</p>
            </div>
          ) : (
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-2">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {note.created_at ? format(new Date(note.created_at), "MMM d, yyyy") : "Unknown date"}
                  </span>
                </div>
            ...
            <div className="space-y-4">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <div>
                  ...
                  <p className="text-base">{currentNote.author_id || "System"}</p>
                </div>
                <div>
                  <label className="text-sm font-medium text-muted-foreground">Date</label>
                  <p className="text-base">
                    {currentNote.created_at ? format(new Date(currentNote.created_at), "PPP") : "Unknown"}
                  </p>
                </div>
              </div>
              {currentNote.updated_at && currentNote.updated_at !== currentNote.created_at && (
                <div>
                  <label className="text-sm font-medium text-muted-foreground">Last Edited</label>
                  <p className="text-sm">{format(new Date(currentNote.updated_at), "PPP 'at' p")}</p>
                </div>
              )}
              <div>
              ...
              <div>
                ...
                <Badge variant={
                  ...
                  {(currentNote.importance || "normal").charAt(0).toUpperCase() + (currentNote.importance || "normal").slice(1)}
                </Badge>
              </div>
            </div>
          ) : (
