...
export default function SuperAdminDashboardHealth() {
  ...
  const overallHealth = dashboards.length > 0 ? (healthyCount / dashboards.length) * 100 : 0;
              ...
              {dashboards.map((dashboard) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="font-medium text-white">{dashboard.dashboard_name}</p>
                    ...
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        {dashboard.last_check 
                          ? formatDistanceToNow(new Date(dashboard.last_check), { addSuffix: true })
                          : 'Never checked'}
                      </p>
                    </div>
                    <Button
                      variant="ghost"
                      size="icon"
                      onClick={() => window.open(dashboard.dashboard_url, '_blank')}
