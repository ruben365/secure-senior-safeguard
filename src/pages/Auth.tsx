...
import { Loader2, Home, Shield, Lock, Mail, User, Eye, EyeOff, CheckCircle2, Sparkles } from "lucide-react";
import { z } from "zod";
...
function Auth() {
      ...
      if (isLogin) {
        ...
        if (data.session && data.user) {
          ...
          const {
            data: profileData,
            error: profileError
          } = await supabase.from("profiles").select("account_status, application_reference").eq("id", data.user.id).single();
  ...
  return <div className="min-h-screen relative overflow-hidden flex flex-col bg-gradient-to-br from-background via-primary/5 to-accent/10">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[600px] h-[600px] bg-primary/10 rounded-full blur-3xl animate-pulse" style={{
        animationDelay: '2s'
      }} />
      </div>
      ...
      {/* Main Content */}
      <div className="relative z-10 w-full max-w-6xl grid lg:grid-cols-2 gap-8 lg:gap-16 items-center animate-fade-in flex-grow py-[52px] my-0 mx-[270px]">
        ...
        <div className="hidden lg:flex flex-col gap-8 animate-scale-in">
          ...
          <Link to="/" className="inline-flex items-center gap-3 group w-fit hover:scale-105 transition-transform duration-300">
            <img src={invisionLogo} alt="InVision Network Shield Logo" className="w-12 h-12 md:w-14 md:h-14 group-hover:scale-110 transition-transform duration-300" />
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-3 gap-4 pt-6">
              ...
              value: "100%",
              label: "Secure"
            }].map((stat, i) => <div key={i} className="space-y-1 p-4 bg-card/50 backdrop-blur-sm rounded-xl border border-border/50 hover:border-primary/50 transition-colors">
            ...
            }].map((feature, i) => <div key={i} className="flex items-center gap-2 text-sm">
        ...
        <div className="w-full max-w-lg mx-auto lg:mx-0 animate-scale-in" style={{
        animationDelay: '0.1s'
      }}>
          {/* Mobile Logo */}
          <Link to="/" className="lg:hidden flex items-center gap-2 md:gap-3 hover:scale-105 transition-transform duration-300 group no-underline mb-8 justify-center">
            <img src={invisionLogo} alt="InVision Network Shield Logo" className="w-8 h-8 md:w-10 md:h-10 group-hover:scale-110 transition-transform duration-300" />
            <div className="flex flex-col leading-tight">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              {!isLogin && <div className="space-y-4">
                  <div className="grid grid-cols-2 gap-3">
                    <Button type="button" variant="outline" className="h-11 bg-background/50" disabled={isLoading}>
              ...
              {!isLogin && <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input id="firstName" value={firstName} onChange={e => setFirstName(e.target.value)} required disabled={isLoading} placeholder="John" className="h-12 pl-10 bg-background/50" />
                    </div>
                  </div>
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-muted-foreground" />
                      <Input id="lastName" value={lastName} onChange={e => setLastName(e.target.value)} required disabled={isLoading} placeholder="Doe" className="h-12 pl-10 bg-background/50" />
                    </div>
                  </div>
                </div>}
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="email" type="email" value={email} onChange={e => setEmail(e.target.value)} required disabled={isLoading} placeholder="name@company.com" className="h-12 pl-10 bg-background/50" />
                </div>
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <Input id="password" type={showPassword ? "text" : "password"} value={password} onChange={e => setPassword(e.target.value)} required disabled={isLoading} placeholder="••••••••" className="h-12 pl-10 pr-12 bg-background/50" />
                ...
                {!isLogin && <p className="text-xs text-muted-foreground flex items-center gap-1.5">
              ...
              {/* Submit Button */}
              <Button type="submit" className="w-full h-13 text-base font-semibold bg-gradient-to-r from-primary to-accent hover:shadow-glow-purple hover:scale-[1.02] transition-all duration-300" disabled={isLoading}>
                {isLoading ? <>
              ...
              <div className="text-center pt-4">
                <p className="text-sm text-muted-foreground">
                  ...
                  <button type="button" onClick={() => navigate(isLogin ? "/signup" : "/auth")} className="text-primary font-semibold hover:underline transition-colors cursor-pointer bg-transparent border-none p-0 inline">
                    {isLogin ? "Apply now" : "Sign in"}
                  </button>
                </p>
                {isLogin && <p className="text-xs text-muted-foreground mt-2">
