...
import { Lock, Shield, Key, Fingerprint, ChevronLeft, ChevronRight, Menu, Search, Bell, Eye, EyeOff } from "lucide-react";
        ...
        <div className="flex items-center justify-between h-full px-4 lg:px-6">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative hidden sm:block max-w-md flex-1">
              ...
              <Input type="text" placeholder="Search settings..." className="pl-10 bg-[#374151] border-gray-700 text-[#F9FAFB] placeholder:text-[#9CA3AF] h-10 rounded-lg" />
            </div>
          </div>
      ...
      <main className={`flex-1 transition-all duration-300 pt-16 w-full ${sidebarOpen ? 'md:ml-[260px]' : 'md:ml-[70px]'}`}>
        <div className="p-6 lg:p-8">
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-3xl font-bold text-[#F9FAFB]">Security Settings</h1>
              <p className="text-[#9CA3AF]">Configure security preferences and policies</p>
            </div>
            <Link to="/admin">
              ...
              <CardContent className="space-y-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <Label className="text-[#F9FAFB]">Two-Factor Authentication</Label>
                    <p className="text-sm text-[#9CA3AF]">Require 2FA for all users</p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-sm text-[#9CA3AF]">Allow fingerprint/face recognition</p>
                  </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-sm text-[#9CA3AF]">Notify on new device logins</p>
                  </div>
              ...
              <CardContent className="space-y-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    ...
                    <p className="text-sm text-[#9CA3AF]">Get notified on detected threats</p>
                  </div>
                  <Switch checked={settings.threatAlerts} onCheckedChange={(v) => setSettings({...settings, threatAlerts: v})} />
                </div>
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <Label className="text-[#F9FAFB]">Auto-Block Threats</Label>
                    <p className="text-sm text-[#9CA3AF]">Automatically block detected threats</p>
                  </div>
                  <Switch checked={settings.autoBlock} onCheckedChange={(v) => setSettings({...settings, autoBlock: v})} />
                </div>
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <Label className="text-[#F9FAFB]">Require VPN</Label>
                    <p className="text-sm text-[#9CA3AF]">Require VPN for remote access</p>
                  </div>
                  <Switch checked={settings.vpnRequired} onCheckedChange={(v) => setSettings({...settings, vpnRequired: v})} />
                </div>
              </CardContent>
            </Card>
