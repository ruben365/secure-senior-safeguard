...
function gradeIcon(grade: string) {
  if (grade === "A") return <ShieldCheck className="w-6 h-6 text-emerald-600" />;
  if (grade === "B") return <ShieldCheck className="w-6 h-6 text-green-500" />;
  if (grade === "C") return <ShieldAlert className="w-6 h-6 text-yellow-500" />;
  if (grade === "D") return <ShieldAlert className="w-6 h-6 text-orange-500" />;
  ...
  return (
    <PageTransition variant="fade">
      <div className="min-h-screen bg-background">
        ...
        <main>
          {/* Hero */}
          <section className="relative overflow-hidden min-h-[60dvh] flex items-center pt-[clamp(100px,14vw,140px)] pb-6">
              ...
              <form onSubmit={handleScan} className="flex flex-col sm:flex-row gap-3 max-w-xl mx-auto">
                <div className="relative flex-1">
                  ...
                  <Input
                    ...
                    className="pl-6 h-7 text-base"
                    disabled={loading}
                  />
                </div>
                <Button type="submit" size="lg" variant="heroPrimary" className="h-7 px-5" disabled={loading}>
                  {loading ? (
                    <>
          ...
          {result && (
            <section className="py-7 animate-fade-in">
              <div className="container mx-auto max-w-3xl">
                {/* Grade card */}
                <Card className={`border-2 mb-4 ${gradeBg(result.grade)}`}>
                  <CardContent className="py-5">
                    <div className="flex flex-col sm:flex-row items-center gap-4">
                      <div className="flex flex-col items-center gap-2">
                        {gradeIcon(result.grade)}
                        <div className={`text-4xl font-black leading-none ${gradeColor(result.grade)}`}>
                          {result.grade}
                        </div>
          ...
          {!result && !loading && (
            <section className="py-10">
              <div className="container mx-auto max-w-4xl">
                  ...
                  {[
                    ...
                    <Card key={title} className="border-border/50 text-center">
                      <CardContent className="pt-5 pb-4">
                        <div className="w-7 h-7 rounded-xl bg-primary/10 flex items-center justify-center mx-auto mb-4">
                ...
                {/* CTA strip */}
                <div className="rounded-2xl bg-gradient-to-br from-primary/10 via-background to-success/10 border border-border/50 p-5 text-center">
          ...
          {result && (
            <section className="py-7 bg-gradient-to-br from-primary/10 via-background to-success/10">
